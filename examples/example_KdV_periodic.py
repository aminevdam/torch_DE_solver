...
def soliton(x, t):
    ...
    s = ((18 * torch.exp((1 / 125) * (t + 25 * x)) * (16 * torch.exp(2 * t) +
                                                      1000 * torch.exp((126 * t) / 125 + (4 * x) / 5) + 9 * torch.exp(
                2 * x) + 576 * torch.exp(t + x) +
                                                      90 * torch.exp((124 * t) / 125 + (6 * x) / 5))) / (
                     5 * (40 * torch.exp((126 * t) / 125) +
...
def soliton_x(x, t):
    ...
    s1 = -(18 * E ** ((1 / 125) * (t + 25 * x)) * (-640 * E ** ((376 * t) / 125) +
                                                   288 * E ** (3 * t + x / 5) - 200000 * E ** (
                                                               (252 * t) / 125 + (4 * x) / 5) +
                                                   81 * E ** ((16 * x) / 5) - 185760 * E ** ((251 * t) / 125 + x) -
                                                   65088 * E ** (2 * t + (6 * x) / 5) - 8100 * E ** (
                                                               (249 * t) / 125 + (7 * x) / 5) +
                                                   225000 * E ** ((127 * t) / 125 + (9 * x) / 5) +
                                                   162720 * E ** ((126 * t) / 125 + 2 * x) + 41796 * E ** (
                                                               t + (11 * x) / 5) -
                                                   405 * E ** (t / 125 + 3 * x) +
                                                   4050 * E ** ((4 / 125) * (31 * t + 75 * x)))) / (
                     25 * (40 * E ** ((126 * t) / 125) +
...
for grid_res in [30, 50, 100]:
    ...
    for _ in range(10):
        ...
        exp_dict_list.append({'grid_res': grid_res, 'time': end - start, 'RMSE': error_rmse.detach().cpu().numpy(),
                              'type': 'kdv_eqn_solitary', 'cache': True})
