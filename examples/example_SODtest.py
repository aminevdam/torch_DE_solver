...
def exact_solution_print(grid, u_exact):
    ...
    ax1.plot_trisurf(grid[:, 0].reshape(-1), grid[:, 1].reshape(-1), net(grid)[:, 0].detach().numpy().reshape(-1),
                     cmap=cm.jet, linewidth=0.2, alpha=1)
    ax2.plot_trisurf(grid[:, 0].reshape(-1), grid[:, 1].reshape(-1), u_exact[:, 1].reshape(-1), cmap='Blues',
                     linewidth=0.2, alpha=1)
    ax2.plot_trisurf(grid[:, 0].reshape(-1), grid[:, 1].reshape(-1), net(grid)[:, 1].detach().numpy().reshape(-1),
                     cmap=cm.jet, linewidth=0.2, alpha=1)
    ax3.plot_trisurf(grid[:, 0].reshape(-1), grid[:, 1].reshape(-1), u_exact[:, 2].reshape(-1), cmap='Blues',
                     linewidth=0.2, alpha=1)
    ax3.plot_trisurf(grid[:, 0].reshape(-1), grid[:, 1].reshape(-1), net(grid)[:, 2].detach().numpy().reshape(-1),
                     cmap=cm.jet, linewidth=0.2, alpha=1)
    ax1.set_xlabel("x")
    ax1.set_ylabel("t")
    ax2.set_xlabel("x")
